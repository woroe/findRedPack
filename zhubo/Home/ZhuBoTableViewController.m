...
@interface ZhuBoTableViewController ()< ZhuBoDetailViewDelegate,JZLPhotoBrowserDelegate,FMGVideoPlayViewDelegate,ZYTagImageViewDelegate>{
...
@property(nonatomic,retain)ZhuBoTableViewCell *currentCell;

@end


static NSInteger page = 1;
@implementation ZhuBoTableViewController
...
- (void)loadUI {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"ZhuBoTableViewCell" bundle:nil] forCellReuseIdentifier:@"ZhuBoTableViewCell"];
...
- (void)loadData {
    ...
    if (_MyZhuboStr.intValue == 0) {
        params[@"BuildSowingType"] = @"GoodNews";
    }
    if (_MyZhuboStr.intValue == 1){ //我的筑播
        params[@"BuildSowingType"] = @"MyNews";
    }
    if (_MyZhuboStr.intValue == 2){
        params[@"BuildSowingType"] = @"UserColloctionNews";
    }
    params[@"Page"] = @(page);
...
- (void)videoplayViewSwitchOrientation:(BOOL)isFull{
    if (isFull) {
        ...
        [self.fullVc dismissViewControllerAnimated:NO completion:^{
            ...
            _fmVideoPlayer.frame = CGRectMake(0,cell.frame.origin.y+imageView.frame.origin.y, SCREEN_WIDTH ,SCREEN_WIDTH +( SCREEN_WIDTH - MARGIN_30 * 2)/2-160*SCREEN_W_SCALE);
