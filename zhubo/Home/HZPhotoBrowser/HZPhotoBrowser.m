...
@interface HZPhotoBrowser() <UIScrollViewDelegate,ZYTagViewDelegate>
...
- (void)setupImageOfImageViewForIndex:(NSInteger)index
{
    ...
    view.imageview.frame = CGRectMake(0, 0, SCREEN_WIDTH, self.view.frame.size.height);
    self.model = [self.delegate photoBrowser:self highQualityImageURLForIndex:index];
    NSString *imgArrStr0 = _model.imgArr[_currentImageIndex];
...
//        return [self.delegate photoBrowser:self highQualityImageURLForIndex:index];
        ...
        return [NSURL URLWithString:ulrStr];
    }
    return nil;
}
