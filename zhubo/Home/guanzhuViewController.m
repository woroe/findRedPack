...
- (void)loadData { //?BuildSowingType=BuildSowing_UserFollowUserList  DqUserId 当前登录用户的ID，Page 页数 PageSize
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    params[@"BuildSowingType"] = @"BuildSowing_UserFollowUserList";
    ...
    [[AFHTTPSessionManager manager] POST:httpurl parameters:params progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        ZBLog(@"BuildSowing_UserFollowUserList--%@", res);
...
-(void)ganzhuAction:(UIButton *)sender{
    ...
    [[AFHTTPSessionManager manager] POST:httpurl parameters:params progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        if([[res objectForKey:@"StatusCode"] isEqual:@(1)]){
            ...
            //刷新关注数据
            [self.delegate gengxinyonghu:dic index:2];
        }else{
            if(Is_Follow_User.intValue == 0){
                btn.backgroundColor = BaseColorYellow;
            } else{
