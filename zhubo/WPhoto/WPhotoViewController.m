...
@interface WPhotoViewController ()<UICollectionViewDelegate, UICollectionViewDataSource,HVideoViewControllerDelegate>{
    ...
    if (![finishbtn.titleLabel.text isEqualToString:finishStr]&&_chooseArray.count) {
        [WPFunctionView finishChoosePhotos:^(NSMutableArray *myChoosePhotoArr) {
            ...
            if (_perAndFanhuiInt == 1) {
                ...
                vc.imageMuArr = myChoosePhotoArr;
                vc.selectImage = [[myChoosePhotoArr objectAtIndex:0] objectForKey:@"image"];
                vc.xiangceAndXiangjiBool = 2;
...
-(void)makeNav {
    ...
    NSString *finishStr = [NSString stringWithFormat:@"0/%ld 完成", (long)_selectPhotoOfMax];
    _rightItem = [navVC viewWithTag:101];
    [_rightItem setTitle:finishStr forState:UIControlStateNormal];
    [navVC setNavViewBack:^{
        [self btnClickBack];
    }];
    [navVC setQuitChooseBack:^{
//        [self quitChoose];
        [self finishChoosePhotos:_rightItem];
    }];
...
//    [_finishBtn setTitle:[NSString stringWithFormat:@"%lu/%ld 完成", choosePhotoCount,(long)_selectPhotoOfMax] forState:UIControlStateNormal];
    [_rightItem setTitle:[NSString stringWithFormat:@"%lu/%ld 完成", choosePhotoCount,(long)_selectPhotoOfMax] forState:UIControlStateNormal];
    NSString *finishStr = [NSString stringWithFormat:@"0/%ld 完成", (long)_selectPhotoOfMax];
...
-(void)btnClickBack
{
    if (_perAndFanhuiInt == 1 || _perAndFanhuiInt ==3 ) {
        ...
        ctrl.delegate = self;
        ctrl.perAndFanhuiInt = 3;
        ctrl.xiangceAndXiangjiBool = 1;
        ctrl.HSeconds = 30;//设置可录制最长时间
        ctrl.addtagArrayCountInt = self.addtagArrayCountInt;
        ctrl.isBool = NO;
...
- (void)geiPlayerUrl:(NSString *)url image:(UIImage *)image{
    [self.delegate getWPHPlayerUrl:url image:image];
//    [self dismissViewControllerAnimated:YES completion:nil];
}
- (void)getImage:(UIImage *)image{
    [self.delegate getWPHImage:image];
//    [self dismissViewControllerAnimated:YES completion:nil];
}
- (void)onCancelAction{
//    [self dismissViewControllerAnimated:YES completion:nil];
