...
#define kBackCornerRadius 5.0                    /** 黑色背景圆角半径 */
...
#pragma mark - private methods   透明度设置
- (void)createSubviews
{
    CAShapeLayer *backLayer = [[CAShapeLayer alloc] init];
    backLayer.fillColor = [[UIColor blackColor] colorWithAlphaComponent:.5].CGColor;
...
- (void)layoutSubviewsWithState:(ZYTagViewState)state arrowPoint:(CGPoint)arrowPoint
{
    ...
    if (state == ZYTagViewStateArrowLeft || state == ZYTagViewStateArrowLeftWithDelete) {
        ...
        self.zy_x = arrowPoint.x - kPointWidth / 2.0;
        // 背景
        [backPath moveToPoint:CGPointMake(kPointWidth+3, self.zy_height / 2.0+1)];
        [backPath addLineToPoint:CGPointMake(kPointWidth+3 , self.zy_height / 2.0-1)];
        
        [backPath addArcWithCenter:CGPointMake(kPointWidth + kPointSpace + kAngleLength+2, 5) radius:5 startAngle:M_PI+M_PI/3 endAngle:0 clockwise:YES];
        
        [backPath addLineToPoint:CGPointMake(kPointWidth + kPointSpace + kAngleLength+4, 0)];
        ...
        [backPath addArcWithCenter:CGPointMake(kPointWidth + kPointSpace + kAngleLength+2 ,self.zy_height  -5) radius:5 startAngle:M_PI_2/2 endAngle:M_PI-M_PI/6 clockwise:YES];
        ...
        self.zy_right = arrowPoint.x + kPointWidth / 2.0;
        // 背景
        [backPath moveToPoint:CGPointMake(self.zy_width - kPointWidth - kPointSpace-1, self.zy_height / 2.0-1)];
        [backPath addLineToPoint:CGPointMake(self.zy_width - kPointWidth - kPointSpace-1, self.zy_height / 2.0+1)];//后加
        
        [backPath addArcWithCenter:CGPointMake(self.zy_width - kAngleLength - kPointWidth - kPointSpace - kBackCornerRadius+5/2, self.zy_height - 5) radius:5 startAngle:M_PI_2-M_PI/3 endAngle:M_PI_2 clockwise:YES];; //后加

        [backPath addLineToPoint:CGPointMake(self.zy_width - kAngleLength - kPointWidth - kPointSpace-2, self.zy_height)];
        ...
        [backPath addLineToPoint:CGPointMake(self.zy_width - kAngleLength - kPointWidth - kPointSpace-2, 0)];
        
        [backPath addArcWithCenter:CGPointMake(self.zy_width - kAngleLength - kPointWidth - kPointSpace - kBackCornerRadius+5/2, 5) radius:5 startAngle:-0 endAngle:-M_PI/3 clockwise:YES];  //后加
...
- (void)updateTitle:(NSDictionary *)info
{
    self.tagInfo.title = [info objectForKey:@"title"];
    self.tagInfo.object = info;
    [self layoutWithTitle:[info objectForKey:@"title"] superview:self.superview];
}
...
- (void)showAnimationWithRepeatCount:(float)repeatCount
{
    ...
    cka2.values =   @[@0.7, @0.9, @0.9, @3.5,  @0.5,  @3.5];
    cka2.keyTimes = @[@0.5, @0.3, @0.3, @0.65, @0.65, @1.0];
    cka2.repeatCount = repeatCount;
    cka2.duration = 1.8;
    self.pointShadowLayer.hidden = NO;
    [self.pointShadowLayer addAnimation:cka2 forKey:@"cka2"];
    
    CAKeyframeAnimation *cka3 = [CAKeyframeAnimation animationWithKeyPath:@"opacity"];
    cka3.values =   @[@1.0, @0.8, @0.6, @0.5,  @0.5,  @0.0];
    cka3.keyTimes = @[@0.0, @0.3, @0.3, @0.65, @0.65, @1];
    cka3.repeatCount = repeatCount;
    cka3.duration = 1.8;
    [self.pointShadowLayer addAnimation:cka3 forKey:@"cka3"];
}
