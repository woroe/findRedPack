...
- (void)addTagWithTitle:(NSString *)title point:(CGPoint)point object:(id)object
{
    ...
    if ([[object objectForKey:@"Position"] isEqualToString:@"right"]) {
        tagInfo.direction = ZYTagDirectionRight;
    }
    if ([[object objectForKey:@"Position"] isEqualToString:@"left"]) {
        tagInfo.direction = ZYTagDirectionLeft;
    }
    ZYTagView *tagView = [[ZYTagView alloc] initWithTagInfo:tagInfo];
    tagView.delegate = self;
    tagView.isEditEnabled = self.isEditEnable;
...
#pragma mark -- 设计所有标签隐藏
- (void)setAllTagsEditEnableYC:(BOOL)isEditEnabled
{
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[ZYTagView class]]) {
            view.hidden = isEditEnabled;
        }
    }
}
